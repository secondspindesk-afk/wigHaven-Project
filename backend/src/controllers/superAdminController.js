...
export default {
    ...
    backupDatabase: async (req, res) => {
        try {
            // OPTIMIZED: Use streaming R2 backup instead of loading all data into memory
            // This prevents memory bombing on servers with limited RAM (512MB)
            const { runBackup } = await import('../jobs/backupJob.js');

            logger.warn(`[SUPER_ADMIN] Database backup triggered by ${req.user?.email || 'Unknown'}`);
            ...
            if (result.success) {
                res.json({
                    success: true,
                    message: 'Backup uploaded to R2 cloud storage',
                    filename: result.filename,
                    stats: result.stats,
                    location: 'Cloudflare R2: wighaven-backups bucket'
                });
            } else {
                res.status(500).json({
                    success: false,
                    error: result.reason || result.error || 'Backup failed',
                    hint: result.reason === 'R2 not configured'
