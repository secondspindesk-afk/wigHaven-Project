...
export const getRecentOrders = async (page = 1, limit = 20) => {
    return smartCache.getOrFetch(
        smartCache.keys.dashboardRecentOrders(page, limit),
        async () => {
...
export const getAdminActivity = async (page = 1, limit = 50) => {
    return smartCache.getOrFetch(
        smartCache.keys.dashboardAdminActivity(page, limit),
        async () => {
            const prisma = getPrisma();
...
export const getLowStockAlerts = async (threshold = 10) => {
    return smartCache.getOrFetch(
        smartCache.keys.dashboardLowStock(threshold),
        async () => {
            const prisma = getPrisma();
